...
$("#update_btn").click(function () {
    $.ajax({
        type: "POST",
        url: "/home/update",
        data: "id=" + x + "|" + $("#a1").val() + "|" + $("#b1").val() + "|" + $("#c1").val() + "|" + $("#d1").val() + "|" + $("#e1").val() + "|" + $("#f1").val() + "|" + $("#g1").val() + "|" + $("#h1").val(),
        dataType: "json",
...
$("#inser_btn").click(function () {
    $.ajax({
        type: "POST",
        url: "/home/inser",
        data: "id=" + $("#a").val() + "|" + $("#b").val() + "|" + $("#c").val() + "|" + $("#d").val() + "|" + $("#e").val() + "|" + $("#f").val(),
        dataType: "json",
        success: function (response) {
            alert("新增成功")
        }
...
$("#save_book").click(function () {
    ...
    for (i = 1; i <= $("#list>li").length; i++) {
        s = document.getElementById(i).innerHTML.split("|");
        if ((s[0] == $("#products1").val() || $("#products1").val() == "") &&
            (s[2] == $("#products2").val() || $("#products2").val() == "") &&
            (s[3] == $("#products3").val() || $("#products3").val() == "") &&
            (s[4] == $("#products4").val() || $("#products4").val() == "")) serch.push({ "Id": s[5], "Name": s[0], "day": s[1], "class": s[2], "status": s[3], "user": s[4] });
    }
    Grid(serch);
    myWindow.data("kendoWindow").close();
});
                ...
                command: [
                    ...
                    },
                    {
                        name: "修改", click: function (e) {
                            $("#serch_window").hide(); $("#inser_window").hide(); $("#update_window").show();
                            e.preventDefault();
                            var tr = $(e.target).closest("tr");
                            var ss = this.dataItem(tr);
                            x = ss.Id;
